...
import { e as deepReady, b as waitAfterFramePaint } from './helpers.js';
...
const Field = proxyCustomElement(class Field extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
function defineCustomElement$1() {
  ...
  const components = ["bal-field"];
  components.forEach(tagName => { switch (tagName) {
    case "bal-field":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, Field);
      }
      break;
  } });
}

const BalField = Field;
const defineCustomElement = defineCustomElement$1;

export { BalField, defineCustomElement };

...
const NavigationLink = proxyCustomElement(class NavigationLink extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.variant = '';
...
function defineCustomElement() {
  ...
  const components = ["bal-nav-link"];
  components.forEach(tagName => { switch (tagName) {
    case "bal-nav-link":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, NavigationLink);
      }
      break;
  } });
}

export { NavigationLink as N, defineCustomElement as d };

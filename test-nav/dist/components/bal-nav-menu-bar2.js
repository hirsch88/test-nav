...
const NavMenuBar = proxyCustomElement(class NavMenuBar extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
...
function defineCustomElement() {
  ...
  const components = ["bal-nav-menu-bar"];
  components.forEach(tagName => { switch (tagName) {
    case "bal-nav-menu-bar":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, NavMenuBar);
      }
      break;
  } });
}

export { NavMenuBar as N, defineCustomElement as d };

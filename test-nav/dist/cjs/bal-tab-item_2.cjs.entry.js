...
const TabItem = class {
  ...
  render() {
    return (index.h(index.Host, { id: this.tabPanelID, role: "tabpanel", "aria-label": this.label, "aria-hidden": !this.isActive ? 'true' : 'false', tabindex: this.isActive ? '0' : '-1', class: {
        'bal-tab-item': true,
...
const TabSelect = ({ items, value, onSelectTab }) => {
  const bemEl = bem.BEM.block('tabs').element('select');
  const onChange = (ev) => {
  ...
  return (index.h("bal-select", { class: Object.assign({}, bemEl.class()), value: value, onBalChange: event => onChange(event) }, items
    .filter(tab => !tab.disabled && !tab.hidden)
    .map(tab => (index.h("bal-select-option", { label: tab.label, value: tab.value }, tab.label)))));
};

const TabIcon = ({ item, inverted, accordion, isAccordionOpen, isMobile, hasBubble, }) => {
  const bemEl = bem.BEM.block('tabs').element('nav').element('item').element('icon');
  let iconColor = item.disabled ? 'grey' : 'primary';
  if (inverted) {
    iconColor = item.disabled ? 'primary-light' : 'white';
  }
  return (index.h("span", { class: Object.assign(Object.assign(Object.assign({}, bemEl.class()), bemEl.modifier('active').class(item.active)), bemEl.modifier('disabled').class(item.disabled)) },
    index.h("bal-icon", { size: isMobile || accordion ? 'small' : '', name: accordion ? 'nav-go-down' : item.icon, color: iconColor, turn: accordion && isAccordionOpen === true && item.active }),
    hasBubble ? (index.h("bal-badge", { class: Object.assign({}, bemEl.element('bubble').class()), size: "small" }, item.bubble)) : ('')));
};

const TabLabel = ({ item, inverted, hasBubble, isVertical, context }) => {
  const bemEl = bem.BEM.block('tabs').element('nav').element('item').element('label');
...
const TabButton = ({ item, tabsId, isFirst, isLast, isMobile, isVertical, accordion, isAccordionOpen, inverted, expanded, spaceless, clickable, iconPosition, context, onSelectTab, }) => {
  const bemEl = bem.BEM.block('tabs').element('nav').element('item');
  if (item.hidden) {
    return;
  }
  ...
  return (index.h(TagType, Object.assign({ id: `${tabsId}-button-${TabButtonIds++}`, role: "tab", class: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, bemEl.class()), bemEl.modifier('active').class(item.active)), bemEl.modifier('disabled').class(item.disabled)), bemEl.modifier('clickable').class(clickable)), bemEl.modifier('accordion').class(accordion)), bemEl.modifier('inverted').class(inverted)), bemEl.modifier('expanded').class(expanded)), bemEl.modifier('spaceless').class(spaceless)), bemEl.modifier('first').class(isFirst)), bemEl.modifier('last').class(isLast)), bemEl.modifier('passed').class(item.passed)), bemEl.modifier('vertical').class(isVertical)), bemEl.modifier(`context-${context}`).class(context !== undefined)), bemEl.modifier(`icon-position-${iconPosition}`).class(iconPosition !== 'horizontal')), { 'bal-focusable': !item.disabled && !item.hidden }), draggable: false, "data-tabs": tabsId, "data-label": item.label, "data-value": item.value, "data-index": item.index, "data-testid": "bal-tabs-item", "aria-selected": item.active ? 'true' : 'false', "aria-disabled": `${item.disabled}`, "aria-label": item.label, "aria-controls": item.tabPanelID }, attrs, { onClick: (ev) => onSelectTab(ev, item) }),
    item.icon ? (index.h(TabIcon, { accordion: false, item: item, isMobile: isMobile, hasBubble: hasIconBubble, inverted: inverted })) : (''),
    index.h(TabLabel, { item: item, isMobile: isMobile, isVertical: isVertical, hasBubble: hasLabelBubble, inverted: inverted, context: context }),
    accordion && !item.href ? (index.h(TabIcon, { accordion: accordion, isAccordionOpen: isAccordionOpen, item: item, isMobile: isMobile, hasBubble: hasAccordionIconBubble, inverted: inverted })) : ('')));
};
let TabButtonIds = 0;
...
const TabNav = ({ items, tabsId, hasCarousel, isVertical, inNavbar, isMobile, isTouch, lineActive, border, accordion, isAccordionOpen, inverted, clickable, animated, spaceless, expanded, verticalColSize, iconPosition, context, onSelectTab, }) => {
  const bemEl = bem.BEM.block('tabs').element('nav');
  const tabs = items.filter(tab => !tab.hidden);
  const isFullHeight = inNavbar && !isTouch;
  const Button = ({ item, index: index$1 }) => (index.h(TabButton, { item: item, tabsId: tabsId, isFirst: index$1 === 0, isLast: index$1 === tabs.length - 1, isMobile: isMobile, isVertical: isVertical, iconPosition: iconPosition, spaceless: spaceless, inverted: inverted, accordion: accordion, isAccordionOpen: isAccordionOpen, context: context, expanded: expanded, clickable: clickable && !item.disabled, onSelectTab: onSelectTab }));
  return (index.h("div", { role: "tablist", id: `${tabsId}-nav`, class: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, bemEl.class()), bemEl.modifier(`full-height`).class(isFullHeight)), bemEl.modifier(`border`).class(border)), bemEl.modifier(`animated`).class(animated)), bemEl.modifier(`vertical`).class(isVertical)), bemEl.modifier(`expanded`).class(expanded && !isVertical)), bemEl.modifier(`vertical-col-${verticalColSize}`).class(isVertical)) },
    hasCarousel ? (index.h("bal-carousel", { id: `${tabsId}-carousel`, class: Object.assign({}, bemEl.element('carousel').class()), fullHeight: isFullHeight, border: border, inverted: inverted, controls: "small", "items-per-view": "auto", steps: 3, onBalChange: formInput.stopEventBubbling },
      tabs.map((tab, index$1) => (index.h("bal-carousel-item", { class: Object.assign(Object.assign({}, bemEl.element('carousel').element('item').class()), bemEl.element('carousel').element('item').modifier('expanded').class(expanded)) },
        index.h(Button, { item: tab, index: index$1 })))),
      index.h("div", { id: `${tabsId}-line`, class: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, bemEl.element('line').class()), bemEl.element('line').modifier(`active`).class(lineActive)), bemEl.element('line').modifier(`inverted`).class(inverted)), bemEl.element('line').modifier(`animated`).class(animated)), bemEl.element('line').modifier(`vertical`).class(isVertical)) }))) : (tabs.map((tab, index$1) => index.h(Button, { item: tab, index: index$1 }))),
    !hasCarousel ? (index.h("div", { id: `${tabsId}-border`, class: Object.assign(Object.assign(Object.assign({}, bemEl.element('border').class()), bemEl.element('border').modifier(`inverted`).class(inverted)), bemEl.element('border').modifier(`vertical`).class(isVertical)) })) : (''),
    !hasCarousel ? (index.h("div", { id: `${tabsId}-line`, class: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, bemEl.element('line').class()), bemEl.element('line').modifier(`active`).class(lineActive)), bemEl.element('line').modifier(`inverted`).class(inverted)), bemEl.element('line').modifier(`animated`).class(animated)), bemEl.element('line').modifier(`vertical`).class(isVertical)) })) : ('')));
};
...
const Tabs = class {
  constructor(hostRef) {
    ...
    this.getOptions = () => {
      if (this.options.length > 0) {
        return [...this.options.map(balTab_util.newBalTabOption)];
      }
      else {
    ...
    this.animateLine = async () => {
      ...
      helpers.raf(async () => {
        await helpers.deepReady(this.el, true);
        this.currentRaf = helpers.raf(async () => {
          ...
          const padding = style.getComputedPadding(target);
          ...
          if (lineElement) {
            ...
            const waitForTransition = helpers.transitionEndAsync(lineElement, 300);
            if (isVertical) {
      ...
      if (this.shouldAnimate()) {
        helpers.raf(() => {
          ...
          this.currentRaf = helpers.raf(async () => {
            ...
            const waitForTransition = helpers.transitionEndAsync(contentEl, 300);
    ...
    this.isTablet = breakpoints_subject.balBreakpoints.isTablet;
    this.store = [];
    this.animated = true;
    this.float = 'left';
  ...
  connectedCallback() {
    this.inNavbar = helpers.hasParent('bal-navbar', this.el);
    if (this.inNavbar) {
    ...
    if (this.accordion) {
      const inNavMenuBar = helpers.hasParent('bal-nav-menu-bar', this.el);
      if (inNavMenuBar) {
        this.isAccordionOpen = false;
      }
      else {
  ...
  listenToWillAnimate(ev) {
    helpers.isChildOfEventTarget(ev, this.el, () => this.animateLine());
  }
  ...
  isUsedInNavbar(ev) {
    ...
    if (parentNavbar && helpers.isDescendant(parentNavbar, this.el)) {
  ...
  render() {
    const block = bem.BEM.block('tabs');
    const isMobile = this.isMobile;
    const isTablet = this.isTablet;
    const isTouch = isMobile || isTablet;
...
__decorate([
  mutation_decorator.ListenToMutation({ tags: ['bal-tabs', 'bal-tab-item'] })

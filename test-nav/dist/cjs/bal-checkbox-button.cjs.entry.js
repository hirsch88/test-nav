...
const BalCheckboxButton = class {
  constructor(hostRef) {
    ...
    this.onClick = (ev) => {
      ...
      if (checkboxEl && targetEl) {
        const isCheckbox = targetEl === checkboxEl || helpers.isDescendant(checkboxEl, targetEl);
        if (!isCheckbox) {
  ...
  render() {
    const block = bem.BEM.block('checkbox-button');
...
__decorate([
  log.Logger('bal-checkbox-button')
...
exports.bal_checkbox_button = BalCheckboxButton;

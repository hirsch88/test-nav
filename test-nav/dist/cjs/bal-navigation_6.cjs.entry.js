...
const observeLevels = (target, level, notify) => {
...
const readSubLevels = async (element, target) => {
...
const Navigation = class {
  constructor(hostRef) {
    ...
    this.previousY = 0;
    this.bodyScrollBlocker = new scroll.BalScrollHandler();
    this.bodyOffset = 0;
    ...
    this.isDesktop = breakpoints_subject.balBreakpoints.isDesktop;
  ...
  handleScroll(_event) {
    if (this.isDesktop && !this.bodyScrollBlocker.isDisabled() && browser.balBrowser.hasWindow) {
  ...
  render() {
    var _a, _b;
    const navigationEl = bem.BEM.block('nav');
    const hasLevels = ((_a = this.levels) === null || _a === void 0 ? void 0 : _a.length) > 0;
    return (index.h(index.Host, { class: Object.assign(Object.assign({}, navigationEl.class()), { 'bal-nav--transformed': this.isMetaHidden }) }, index.h("bal-navigation-meta", { class: "is-hidden-touch", "aria-label-meta": this.ariaLabelMeta }, index.h("bal-navigation-meta-start", null, hasLevels && (index.h("bal-tabs", { spaceless: true, inverted: true, context: "meta", value: this.selectedMetaValue }, this.levels.map((meta, index$1) => {
      return meta.isTabLink ? (index.h("bal-tab-item", Object.assign({ label: meta.label, value: meta.value, href: meta.link, target: meta.target }, meta.trackingData))) : (index.h("bal-tab-item", Object.assign({ label: meta.label, value: meta.value }, meta.trackingData, { onBalNavigate: ev => {
          meta.onClick(ev.detail);
          this.selectedMetaValue = meta.value;
          this.selectedMetaIndex = index$1;
          this.isMainBodyOpen = false;
          this.selectedMainValue = '';
        } })));
    })))), index.h("bal-navigation-meta-end", null, this.levels && index.h("slot", { name: "meta-actions" }))), index.h("bal-navigation-main", { class: {
        'is-hidden-touch': true,
        'container': true,
        'bal-nav__main--expanded': this.isMainBodyOpen,
      }, ref: el => {
        this.mainNavElement = el;
      }, "aria-label-main": this.ariaLabelMain }, index.h("bal-navigation-main-head", { slot: "main-head", class: {
        'is-hidden-mobile': true,
        'bal-nav__main__head--active': this.isMainBodyOpen,
      } }, index.h("div", null, index.h("a", { href: this.logoPath, class: "bal-nav__main-head-logo", tabindex: -1 }, index.h("bal-logo", { color: "blue", animated: this.logoAnimated && !this.isTouch })), index.h("bal-tabs", { accordion: true, spaceless: true, context: "navigation", value: this.selectedMainValue, onBalChange: event => this.onMainTabChange(event), ref: el => {
        this.mainNavTabsEl = el;
      } }, hasLevels &&
      ((_b = this.levels[this.selectedMetaIndex].subLevels) === null || _b === void 0 ? void 0 : _b.map((main, index$1) => {
        return main.isTabLink ? (index.h("bal-tab-item", { label: main.label, value: main.value, href: main.link, target: main.target, onBalNavigate: ev => {
            main.onClick(ev.detail);
            this.selectedMainIndex = index$1;
            this.isMainBodyOpen = false;
            this.selectedMainValue = main.value;
          } })) : (index.h("bal-tab-item", Object.assign({ label: main.label, value: main.value }, main.trackingData, { onBalNavigate: ev => {
            main.onClick(ev.detail);
            this.selectedMainIndex = index$1;
      ...
      }))))), this.isMainBodyOpen && (index.h("bal-navigation-main-body", { slot: "main-body" }, this.levels
      .filter((_, index) => index === this.selectedMetaIndex)
      .map(meta => {
      var _a;
      return (_a = meta.subLevels) === null || _a === void 0 ? void 0 : _a.filter((_, mainIndex) => this.selectedMainIndex === mainIndex).map(main => (index.h("bal-navigation-menu", { "link-href": main.link, "link-name": main.linkLabel, target: main.target, elements: main.subLevels, tracking: main.trackingData })));
    })))), index.h("div", { class: "bal-nav__metamobile container" }, index.h("nav", { role: "navigation", "aria-label": this.ariaLabelMeta }, index.h("a", { href: this.logoPath, class: "bal-nav__main-mobile__logo", tabindex: -1 }, index.h("bal-logo", { color: "blue", animated: this.logoAnimated && this.isTouch })), index.h("div", { class: "bal-nav__metamobile__actions" }, index.h("slot", { name: "meta-actions-mobile" })), index.h("bal-button", { slot: "burger", "data-testid": "navigation-burger", color: this.isMainBodyOpen ? 'primary' : 'light', square: true, icon: this.isMainBodyOpen ? 'close' : 'menu-bars', onClick: () => this.onBurgerButtonClick() }))), index.h("div", { class: "bal-nav__main-mobile container", style: {
        '--bal-nav-main-mobile-height': this.mainMobileHeight,
        'display': this.isMainBodyOpen && this.isTouch ? 'block' : 'none',
      } }, index.h("bal-list", { border: true, "accordion-one-level": true, size: "small", class: "bal-nav__main__list" }, this.levels.map(meta => {
      var _a;
      return (index.h("bal-list-item", { accordion: true, class: "bal-nav__main-mobile__main-accordion" }, index.h("bal-list-item-accordion-head", { icon: "nav-go-down" }, index.h("bal-list-item-content", null, index.h("bal-list-item-title", { level: "large" }, meta.label))), index.h("bal-list-item-accordion-body", { class: "bal-list__item__accordion-body__parent" }, meta.link && (index.h("div", { class: "bal-nav__main-mobile__link" }, index.h("a", Object.assign({ href: meta.link }, meta.trackingData), meta.linkLabel))), index.h("bal-list", { "accordion-one-level": true, size: "small" }, (_a = meta.subLevels) === null || _a === void 0 ? void 0 : _a.map(main => {
        return main.isTabLink ? (index.h("bal-list-item", { "sub-accordion-item": true, href: main.link, target: main.target }, index.h("bal-list-item-content", null, index.h("bal-list-item-title", { level: "medium" }, main.label)))) : (index.h("bal-list-item", { accordion: true, "sub-accordion-item": true }, index.h("bal-list-item-accordion-head", { icon: "nav-go-down" }, index.h("bal-list-item-content", null, index.h("bal-list-item-title", { level: "medium" }, main.label))), index.h("bal-list-item-accordion-body", null, index.h("bal-navigation-menu", { "link-href": main.link, "link-name": main.linkLabel, target: main.target, elements: main.subLevels, tracking: main.trackingData }))));
      })))));
    }))), index.h("div", { class: "bal-nav__foot-mobile", style: {
        display: this.isMainBodyOpen && this.isTouch ? 'block' : 'none',
      } }, index.h("slot", { name: "meta-mobile-foot" })), index.h("slot", null)));
  }
...
const NavigationLevels = class {
  ...
  render() {
    return (index.h(index.Host, { class: "is-hidden" }, index.h("slot", null)));
  }
  get el() { return index.getElement(this); }
};

exports.bal_navigation = Navigation;
exports.bal_navigation_level_block = NavigationLevelBlock;
exports.bal_navigation_level_block_item = NavigationLevelBlockItem;
exports.bal_navigation_level_main = NavigationLevelMain;
exports.bal_navigation_level_meta = NavigationLevelMeta;
exports.bal_navigation_levels = NavigationLevels;

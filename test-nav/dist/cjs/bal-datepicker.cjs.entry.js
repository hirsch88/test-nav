...
const index$2 = require('./index-843a2913.js');
...
const Datepicker = class {
  constructor(hostRef) {
    ...
    this.onInput = (ev) => {
      ...
      if (input) {
        ...
        if (this.inputValue && this.inputValue.length >= 6) {
          const date = index_esm.parse(this.inputValue, this.getLocale());
          const dateString = index_esm.formatDateString(date);
          if (index_esm.isValidIsoString(dateString)) {
    ...
    this.onInputChange = (ev) => {
      ...
      const date = index_esm.parse(inputValue, this.getLocale());
      const dateString = index_esm.formatDateString(date);
      const formattedValue = index_esm.format(this.getLocale(), date);
    ...
    this.onInputKeyUp = (ev) => {
      if (index_esm.isSpaceKey(ev) && !this.triggerIcon) {
        if (this.isPopoverOpen) {
          this.close();
        }
        else {
      ...
      if (index_esm.isEnterKey(ev) && !this.triggerIcon) {
        const date = index_esm.parse(this.nativeInput.value, this.getLocale());
        const dateString = index_esm.formatDateString(date);
        if (this.isPopoverOpen) {
    ...
    this.onInputKeyDown = (ev) => {
      const separator = index_esm.dateSeparator(this.getLocale());
      const allowedKeys = [...keys_constant.NUMBER_KEYS, separator, ...keys_constant.ACTION_KEYS];
      if (!keys_constant.isCtrlOrCommandKey(ev) && allowedKeys.indexOf(ev.key) < 0) {
        ev.preventDefault();
        ev.stopPropagation();
      }
      if (ev.key === 'Tab') {
    ...
    this.onYearSelect = (ev) => {
      ...
      let month = undefined;
      if (this.defaultDate) {
        const defaultDate = index_esm.parse(this.defaultDate);
        if (this.max) {
          const maxDate = index_esm.parse(this.max);
          if (defaultDate.getMonth() > maxDate.getMonth()) {
            month = maxDate.getMonth();
          }
        }
        if (this.min) {
          const minDate = index_esm.parse(this.min);
    ...
    this.selectedDate = '';
    this.pointerDate = {
      year: getYear(index_esm.now()),
      month: getMonth(index_esm.now()),
      day: getDate(index_esm.now()),
    };
    ...
    if (formElement === null || formElement === void 0 ? void 0 : formElement.contains(this.el)) {
      ...
      this.resetHandlerTimer = setTimeout(() => {
        if (this.initialValue) {
          this.nativeInput.value = index_esm.format(this.getLocale(), index_esm.parse(this.initialValue));
        }
        else {
  ...
  async select(dateString) {
    var _a;
    this.nativeInput.value = index_esm.format(this.getLocale(), index_esm.parse(dateString));
    this.updateValue(dateString);
    this.updatePointerDates();
  ...
  updatePointerDates() {
    let date = index_esm.now();
    date.setDate(1);
    if (this.selectedDate) {
      date = index_esm.parse(this.selectedDate);
    }
    else {
      if (this.defaultDate) {
        date = index_esm.parse(this.defaultDate);
      }
    }
    if (this.min) {
      const minDate = index_esm.parse(this.min);
  ...
  updateValue(dateString, isHuman = true) {
    ...
    if (this.value !== dateString) {
      ...
      if (isHuman && (this.isDateInRange(index_esm.parse(this.value)) || this.value === '')) {
  ...
  get minYear() {
    ...
    return this.minYearProp ? this.minYearProp : getYear(subYears(index_esm.now(), 100));
  }
  get maxYear() {
    if (this.max) {
      return parseInt(this.max.substring(0, 4), 10);
    }
    return this.maxYearProp ? this.maxYearProp : getYear(addYears(index_esm.now(), 100));
  }
  get years() {
    let years = Array.from({ length: this.maxYear - this.minYear + 1 }, (_, index) => this.minYear + index);
    if (this.min && this.pointerDate.month === getMonth(index_esm.parse(this.min))) {
      const minYear = getYear(index_esm.parse(this.min));
      years = years.filter(y => y >= minYear);
    }
    if (this.max && this.pointerDate.month === getMonth(index_esm.parse(this.max))) {
      const maxYear = getYear(index_esm.parse(this.max));
      years = years.filter(y => y <= maxYear);
    }
    return years;
  }
  get months() {
    const monthNames = balDatepicker_i18n.i18nBalDatepicker[this.language].monthsShort;
    let months = monthNames.map((name, index) => ({ name, index }));
    if (this.min && this.pointerDate.year === getYear(index_esm.parse(this.min))) {
      const minMonth = parseInt(this.min.substring(5, 7), 10) - 1;
      months = months.filter(month => month.index >= minMonth);
    }
    if (this.max && this.pointerDate.year === getYear(index_esm.parse(this.max))) {
  ...
  get weekDays() {
    const translations = [...balDatepicker_i18n.i18nBalDatepicker[this.language].weekdaysMin];
  ...
  getLocale() {
    const config = index$2.useBalConfig();
      ...
      do {
        row = [
          ...row,
          {
            ...
            display: index_esm.format(this.getLocale(), dayDatePointer),
            dateString: index_esm.formatDateString(dayDatePointer),
            label: getDate(dayDatePointer).toString(),
            isToday: isSameDay(dayDatePointer, index_esm.now()),
            isSelected: index_esm.parse(this.selectedDate) !== undefined &&
              isSameDay(dayDatePointer, index_esm.parse(this.selectedDate)),
  ...
  getAllowedDates(dayDatePointer) {
    if (index$1.lodash_isnil(this.allowedDates)) {
      return true;
    }
    return this.allowedDates(index_esm.formatDateString(dayDatePointer));
  }
  formatDate(value) {
    const separator = index_esm.dateSeparator(this.getLocale());
  ...
  render() {
    const block = bem.BEM.block('datepicker');
    ...
    return (index.h(index.Host, { onClick: this.handleClick, "aria-disabled": this.disabled ? 'true' : null, class: Object.assign(Object.assign({}, block.class()), block.modifier('disabled').class(this.disabled || this.readonly)) }, index.h("input", { type: "date", class: Object.assign({}, native.class()), name: this.name, min: this.min, max: this.max, value: this.value, tabindex: -1, "aria-hidden": "true" }), index.h("bal-popover", { onBalChange: this.onPopoverChange, ref: el => (this.popoverElement = el) }, this.renderInput(), index.h("bal-popover-content", { spaceless: true, contentMinWidth: 392 }, index.h("div", { class: Object.assign({}, popup.class()) }, this.renderHeader(), index.h("div", { class: Object.assign({}, popupBody.class()) }, this.renderGrid()), index.h("div", { class: Object.assign({}, popupFooter.class()) }, index.h("slot", null)))))));
  }
  renderInput() {
    return (index.h("div", { "bal-popover-trigger": true, class: "control" }, index.h("bal-input-group", { disabled: this.disabled || this.readonly, invalid: this.invalid }, index.h("input", Object.assign({ class: {
        'input': true,
        'data-test-input': true,
        'is-clickable': !this.disabled && !this.triggerIcon && !this.readonly,
        'is-disabled': this.disabled || this.readonly,
        'is-danger': this.invalid,
      }, "data-testid": "bal-datepicker-input", ref: el => (this.nativeInput = el), id: this.ariaForm.controlId || this.inputId, "aria-labelledby": this.ariaForm.labelId, "aria-describedby": this.ariaForm.messageId, "aria-invalid": this.invalid === true ? 'true' : 'false', "aria-disabled": this.disabled ? 'true' : null, type: "text", maxlength: "10", autoComplete: "off", value: index_esm.format(this.getLocale(), index_esm.parse(this.value || '')), required: this.required, disabled: this.disabled, readonly: this.readonly, placeholder: this.placeholder, onKeyDown: e => this.onInputKeyDown(e), onKeyUp: e => this.onInputKeyUp(e), onInput: this.onInput, onClick: this.onInputClick, onChange: this.onInputChange, onBlur: this.onInputBlur, onFocus: this.onInputFocus }, this.inheritedAttributes)), !this.loading ? (index.h("bal-icon", { class: {
  ...
  renderGrid() {
    const block = bem.BEM.block('datepicker-grid');
    const rowEl = block.element('row');
    const cellEl = block.element('cell');
    return (index.h("div", null, this.renderWeekDayHeader(), index.h("div", { class: Object.assign({}, block.class()) }, this.calendarGrid.map(row => (index.h("div", { class: Object.assign({}, rowEl.class()) }, row.map(cell => (index.h("button", { type: "button", "data-date": cell.dateString, onClick: () => this.onClickDateCell(cell), disabled: cell.isDisabled, class: Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, cellEl.class()), { 'button': true, 'is-text': !cell.isDisabled && !cell.isSelected, 'is-primary': cell.isSelected && cell.isSelected, 'is-disabled': cell.isDisabled || cell.isOutdated }), cellEl.modifier('today').class(cell.isToday)), cellEl.modifier('selectable').class(!cell.isDisabled && !cell.isOutdated)), cellEl.modifier('disabled').class(cell.isDisabled || cell.isOutdated)), cellEl.modifier('outdated').class(cell.isOutdated)), cellEl.modifier('selected').class(cell.isSelected)) }, index.h("span", null, cell.label))))))))));
  }
  renderWeekDayHeader() {
    const block = bem.BEM.block('datepicker-grid');
    const headerEl = block.element('header');
    const cellEl = block.element('cell');
    return (index.h("header", { class: Object.assign({}, headerEl.class()) }, this.weekDays.map(weekday => (index.h("div", { class: Object.assign(Object.assign({}, cellEl.class()), cellEl.modifier('header').class()) }, index.h("span", null, weekday))))));
  }
  renderHeader() {
    const block = bem.BEM.block('datepicker-pagination');
    const innerEl = block.element('inner');
    const monthAndYearEl = block.element('month-and-year');
    const selectEl = monthAndYearEl.element('select');
    return (index.h("header", { class: Object.assign({}, block.class()) }, index.h("div", { class: Object.assign({}, innerEl.class()) }, index.h("bal-button", { square: true, size: this.isMobile ? 'small' : '', color: "info", icon: "nav-go-left", disabled: this.isPreviousMonthDisabled, onClick: () => this.previousMonth() }), index.h("div", { class: Object.assign({}, monthAndYearEl.class()) }, index.h("div", { class: Object.assign(Object.assign({}, selectEl.class()), selectEl.modifier('month').class()) }, index.h("div", { class: "select" }, index.h("select", { onInput: this.onMonthSelect }, this.months.map(month => (index.h("option", { value: month.index, selected: this.pointerDate.month === month.index }, month.name)))))), index.h("div", { class: Object.assign(Object.assign({}, selectEl.class()), selectEl.modifier('year').class()) }, index.h("div", { class: "select" }, index.h("select", { onInput: this.onYearSelect }, this.years.map(year => (index.h("option", { value: year, selected: this.pointerDate.year === year }, year))))))), index.h("bal-button", { square: true, size: this.isMobile ? 'small' : '', color: "info", icon: "nav-go-right", disabled: this.isNextMonthDisabled, onClick: () => this.nextMonth() }))));
  }
  previousMonth() {
  ...
  isDateInRange(cellDate) {
    const parsedCellDate = index_esm.parse(index_esm.formatDateString(cellDate));
    if (this.min && this.max && this.max > this.min) {
      return isWithinInterval(parsedCellDate, {
        start: index_esm.parse(this.min),
        end: index_esm.parse(this.max),
      });
    }
    if (this.min) {
      return isAfter(parsedCellDate, index_esm.parse(this.min)) || isSameDay(parsedCellDate, index_esm.parse(this.min));
    }
    if (this.max) {
      return (isBefore(parsedCellDate, addDays(index_esm.parse(this.max), 1)) ||
        isSameDay(parsedCellDate, index_esm.parse(this.max)));
    }
    return true;
  }

...
import { p as parse$1, f as formatDateString, i as isValidIsoString, b as format$1, c as isSpaceKey, d as isEnterKey, e as dateSeparator, n as now } from './index.esm-d25206f6.js';
import { l as lodash_isnil } from './index-82aff103.js';
import { N as NUMBER_KEYS, A as ACTION_KEYS, i as isCtrlOrCommandKey } from './keys.constant-a195c4b1.js';
import { i as i18nBalDatepicker } from './bal-datepicker.i18n-590bae0f.js';
import { c as defaultConfig, j as useBalConfig, k as defaultLocale, L as ListenToConfig } from './index-6394c1a6.js';
import { s as stopEventBubbling, g as getInputTarget, i as inputHandleFocus, a as inputHandleHostClick, b as inputListenOnClick, c as inputSetFocus, d as inputSetBlur } from './form-input-2b8d78bf.js';
...
const Datepicker = class {
  constructor(hostRef) {
    ...
    this.onInput = (ev) => {
      ...
      if (input) {
        ...
        if (this.inputValue && this.inputValue.length >= 6) {
          const date = parse$1(this.inputValue, this.getLocale());
    ...
    this.onInputChange = (ev) => {
      ...
      const date = parse$1(inputValue, this.getLocale());
      const dateString = formatDateString(date);
      const formattedValue = format$1(this.getLocale(), date);
    ...
    this.onInputKeyUp = (ev) => {
      ...
      if (isEnterKey(ev) && !this.triggerIcon) {
        const date = parse$1(this.nativeInput.value, this.getLocale());
        const dateString = formatDateString(date);
        if (this.isPopoverOpen) {
    ...
    this.onYearSelect = (ev) => {
      ...
      let month = undefined;
      if (this.defaultDate) {
        const defaultDate = parse$1(this.defaultDate);
        if (this.max) {
          const maxDate = parse$1(this.max);
          if (defaultDate.getMonth() > maxDate.getMonth()) {
            month = maxDate.getMonth();
          }
        }
        if (this.min) {
          const minDate = parse$1(this.min);
    ...
    if (formElement === null || formElement === void 0 ? void 0 : formElement.contains(this.el)) {
      ...
      this.resetHandlerTimer = setTimeout(() => {
        if (this.initialValue) {
          this.nativeInput.value = format$1(this.getLocale(), parse$1(this.initialValue));
        }
        else {
  ...
  async select(dateString) {
    var _a;
    this.nativeInput.value = format$1(this.getLocale(), parse$1(dateString));
    this.updateValue(dateString);
    this.updatePointerDates();
  ...
  updatePointerDates() {
    ...
    if (this.selectedDate) {
      date = parse$1(this.selectedDate);
    }
    else {
      if (this.defaultDate) {
        date = parse$1(this.defaultDate);
      }
    }
    if (this.min) {
      const minDate = parse$1(this.min);
  ...
  updateValue(dateString, isHuman = true) {
    ...
    if (this.value !== dateString) {
      ...
      if (isHuman && (this.isDateInRange(parse$1(this.value)) || this.value === '')) {
  ...
  get years() {
    ...
    if (this.min && this.pointerDate.month === getMonth(parse$1(this.min))) {
      const minYear = getYear(parse$1(this.min));
      years = years.filter(y => y >= minYear);
    }
    if (this.max && this.pointerDate.month === getMonth(parse$1(this.max))) {
      const maxYear = getYear(parse$1(this.max));
  ...
  get months() {
    ...
    if (this.min && this.pointerDate.year === getYear(parse$1(this.min))) {
      const minMonth = parseInt(this.min.substring(5, 7), 10) - 1;
      months = months.filter(month => month.index >= minMonth);
    }
    if (this.max && this.pointerDate.year === getYear(parse$1(this.max))) {
      ...
      do {
        row = [
          ...row,
          {
            ...
            display: format$1(this.getLocale(), dayDatePointer),
            dateString: formatDateString(dayDatePointer),
            label: getDate(dayDatePointer).toString(),
            isToday: isSameDay(dayDatePointer, now()),
            isSelected: parse$1(this.selectedDate) !== undefined &&
              isSameDay(dayDatePointer, parse$1(this.selectedDate)),
  ...
  isDateInRange(cellDate) {
    const parsedCellDate = parse$1(formatDateString(cellDate));
    if (this.min && this.max && this.max > this.min) {
      return isWithinInterval(parsedCellDate, {
        start: parse$1(this.min),
        end: parse$1(this.max),
      });
    }
    if (this.min) {
      return isAfter(parsedCellDate, parse$1(this.min)) || isSameDay(parsedCellDate, parse$1(this.min));
    }
    if (this.max) {
      return (isBefore(parsedCellDate, addDays(parse$1(this.max), 1)) ||
        isSameDay(parsedCellDate, parse$1(this.max)));
    }
    return true;
  }
